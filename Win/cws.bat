...
if NOT EXIST %EXDIR%%DWS%\%1.%EXT% (
	if "%2" == "" (
		ECHO #include ^<iostream^>>> %EXDIR%%DWS%\%1.%EXT%
		ECHO.>> %EXDIR%%DWS%\%1.%EXT%
		ECHO #define show^(x^) std::cout ^<^< x ^<^< std::endl>> %EXDIR%%DWS%\%1.%EXT%
		ECHO.>> %EXDIR%%DWS%\%1.%EXT%
...
IF "%FARGV%" == "new" (
	FOR %%i IN (%ARGV%) DO (
		IF EXIST %EXDIR%%DWS%\%%i.%EXT% (
			...
			ECHO #include ^<iostream^>>> %EXDIR%%DWS%\%%i.%EXT%
			ECHO.>> %EXDIR%%DWS%\%%i.%EXT%
			ECHO #define show^(x^) std::cout ^<^< x ^<^< std::endl>> %EXDIR%%DWS%\%%i.%EXT%
			ECHO.>> %EXDIR%%DWS%\%%i.%EXT%
